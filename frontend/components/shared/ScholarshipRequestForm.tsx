...
export default function ScholarshipRequestForm({ variant = "light", onSuccess }: ScholarshipRequestFormProps) {
    ...
    if (status === "success") {
        return (
            <motion.div
                ...
                className={`rounded-3xl p-8 text-center flex flex-col items-center justify-center min-h-[400px] ${variant === 'glass' || variant === 'dark' ? 'bg-white/5 text-white' : 'bg-white text-slate-900 border border-slate-100 shadow-xl'
                    }`}
            >
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 animate-bounce ${variant === 'glass' || variant === 'dark' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-600'
                    }`}>
                    <CheckCircle size={32} />
                </div>
                <h3 className="text-2xl font-black mb-2">Request Received!</h3>
                <p className={`mb-8 max-w-sm mx-auto text-sm ${variant === 'glass' || variant === 'dark' ? 'text-white/70' : 'text-slate-500'}`}>
                    We'll check scholarship options for your preferred college and contact you shortly.
                </p>
                <button
                    onClick={() => setStatus("idle")}
                    className={`font-bold py-3 px-8 rounded-xl transition-all ${variant === 'glass' || variant === 'dark' ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'
                        }`}
                >
    ...
    const inputClasses = variant === 'glass'
        ...
        : variant === 'dark'
            ? "w-full bg-white border border-white shadow-sm rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-brand-magenta/30 transition-all font-bold text-slate-900 placeholder:text-slate-400"
    ...
    const labelClasses = `block text-[10px] font-bold uppercase tracking-wider mb-1.5 ${variant === 'glass' || variant === 'dark' ? 'text-white/80' : 'text-slate-500'}`;
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
                ...
                className={`w-full font-black py-4 rounded-xl shadow-lg transition-all transform hover:-translate-y-0.5 active:scale-[0.98] flex items-center justify-center gap-2 mt-4 uppercase tracking-widest text-sm ${variant === 'glass' || variant === 'dark'
                    ? 'bg-brand-magenta hover:bg-brand-magenta/90 text-white shadow-brand-magenta/20'
